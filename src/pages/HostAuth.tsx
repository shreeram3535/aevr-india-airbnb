...
export const HostAuth = () => {
    ...
    // Google Auth Handler for Hosts
    const handleGoogleLogin = async () => {
        setMessage(null);
        try {
            localStorage.setItem('aevr.oauth_role', 'host');
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const { error, data } = await authService.signUp(email, password, fullName, 'host');
            if (error) {
                setMessage(error.message);
                setMessageType('error');
                return;
            ...
            if (data.session) {
                ...
                setMessageType('info');
                setMessage('Account created! Please check your email to verify your account, then come back to sign in.');
            }
        } catch (error) {
            ...
            <div className={styles.rightPanel}>
                ...
                <div className={styles.formCard}>
                    ...
                    <button 
                        ...
                        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', padding: '12px', backgroundColor: '#fff', border: '1px solid #E2E8F0', borderRadius: '8px', cursor: 'pointer', fontWeight: 500, fontSize: '0.95rem', marginBottom: '20px', color: '#1e293b' }}
                    >
